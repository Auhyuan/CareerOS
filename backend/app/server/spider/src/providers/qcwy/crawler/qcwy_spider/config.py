...
def load_env(env_path=ENV_PATH):
    """读取爬虫目录下的 .env 文件，并写入当前进程环境变量。"""
    if not env_path.exists():
        return

    # 这里保留轻量解析逻辑，方便 crawler 独立调试；正式 API 调用会由 provider 直接传入配置。
...
def resolve_output_dir(path_text):
    """把输出目录解析为绝对路径；相对路径默认位于 crawler 目录下。"""
...
class SpiderConfig:
    ...
    def from_cli(cls):
        ...
        parser.add_argument("--pages", type=int, help="每个关键词和城市组合采集页数")
        ...
        # 命令行没有传值时回退到 .env；.env 没有传值时使用保守默认值。
