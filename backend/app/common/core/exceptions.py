...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def business_exception_handler(request: Request, error: BusinessException):
        ...
        return JSONResponse(
            status_code=200,
            content=Result.fail(error.code, error.msg).model_dump(),
        )
    ...
    async def http_exception_handler(request: Request, error: HTTPException):
        ...
        return JSONResponse(
            ...
            content=Result.fail(error.status_code, str(error.detail)).model_dump(),
        )
    ...
    async def request_validation_exception_handler(request: Request, error: RequestValidationError):
        ...
        return JSONResponse(
            status_code=422,
            content=Result.fail(422, format_validation_error(error.errors())).model_dump(),
        )
    ...
    async def unhandled_exception_handler(request: Request, error: Exception):
        ...
        return JSONResponse(
            status_code=500,
            content=Result.fail(500, f"服务内部错误: {str(error)}").model_dump(),
        )


def format_validation_error(errors: list[dict]) -> str:
    """
    把 FastAPI/Pydantic 的校验错误整理成简洁中文提示。
