...
def create_app() -> FastAPI:
    """Create the capability-backend FastAPI application."""
    app = FastAPI(lifespan=app_lifespan)
    app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=False, allow_methods=["*"], allow_headers=["*"])
    register_exception_handlers(app)
    app.include_router(agent_router, prefix="/agent", tags=["agent capability"])
...
def print_routes(app: FastAPI) -> None:
    ...
    print("Registered capability-backend routes:")
    for route in app.routes:
        print(route.path if hasattr(route, "path") else route)


def print_startup_banner() -> None:
    """Print a readable startup banner for capability-backend."""
    host = os.getenv("FASTAPI_HOST", "127.0.0.1")
    port = os.getenv("FASTAPI_PORT", "8090")
    print(f"capability-backend starting: http://{host}:{port}")
